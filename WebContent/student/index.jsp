...
<body onload="sT()">
	<table width="1003" height="100%" border="0" cellpadding="0"
				...
				<form action="submitExam" method="post">
					<table width="98%" height="100%"  border="0" align="center" cellpadding="0"
						...
						<s:iterator value="#request.subjects" var="subject" status="sta">
							<tr>
								<td colspan="3"><input type="hidden" name="subjectID" value="${subject.subjectID}" /><strong>第<span class="STYLE5">${sta.index + 1}</span>题&nbsp;${subject.subjectTitle}
								</strong></td>
							</tr>
							<tr>
							...
							<tr>
								<td height="32" colspan="3" bgcolor="#CCCCCC">选择答案： <input
									type="radio" name="subjectAnswer<%=i%>" value="A" /> A <input
									type="radio" name="subjectAnswer<%=i%>" value="B" /> B <input
									type="radio" name="subjectAnswer<%=i%>" value="C" /> C <input
									type="radio" name="subjectAnswer<%=i%>" value="D" /> D
								</td>
							</tr>
							<%
								i++;
